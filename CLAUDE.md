...
结局页现在不再只显示“结局标题 + 数字 + 文案”。

新增了一个 `ending-diagnosis` 模块，拆成两张卡：
- 第一张：失败原因 / 主要死因
- 第二张：生存代价 / 被压到极限的资源

`buildEndingDiagnosis()` 会根据：
- ending id
- 最终资源状态
- `getResourceStory()` 返回的 strongest / weakest resource
...
这一层的目标是：让结局真正回答“你到底死在哪里”，而不是只给玩家一组数字让他自己猜。
